...
export const Route = createFileRoute("/create-qp")({
  head: () => ({
    meta: [
      { title: "Create Question Paper — SBM Slides" },
      { name: "description", content: "Design beautiful question papers in the SBM Slides editor." },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: CreateQPPage,
});
...
export default function CreateQPPage() {
  ...
  const [presentation, setPresentation] = useState<Presentation>(() => {
    ...
    p.name = "Untitled Question Paper";
    p.slides[0].data = null;
    return p;
  });
  ...
  }, [loadSlide, pushHistory, presentation.slides]);
  ...
  useEffect(() => {
    const resize = () => {
      ...
      const fit = Math.min(availW / QP_W, availH / QP_H);
      const s = fit * zoom;
      const c = fabricRef.current;
      c.setDimensions({ width: QP_W * s, height: QP_H * s });
      c.setZoom(s);
    };
    resize();
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      const thumb = canvas.toDataURL({ format: "png", multiplier: 0.1 });
  ...
  const addHeading = () => add(new fabric.Textbox("School Name / Exam Title", {
    left: QP_W / 2, top: 80, width: 600, fontSize: 32, fontWeight: "700",
    fill: "#000000", fontFamily: "Inter", textAlign: "center", originX: "center"
  }));
  const addSubheading = () => add(new fabric.Textbox("Subject: \nTime: 3 Hours", {
    left: 80, top: 160, width: 300, fontSize: 18, fontWeight: "600",
    fill: "#000000", fontFamily: "Inter"
  }));
  const addParagraph = () => add(new fabric.Textbox("1. Answer all questions.", {
    left: 80, top: 220, width: QP_W - 160, fontSize: 14, fill: "#000000", fontFamily: "Inter",
  }));
  const addBullets = () => add(new fabric.Textbox("• First question\n• Second question", {
    left: 80, top: 260, width: QP_W - 160, fontSize: 14, fill: "#000000", fontFamily: "Inter",
  }));
  const addRect = () => add(new fabric.Rect({
    left: QP_W / 2, top: QP_H / 2, width: 200, height: 100, fill: "#e5e7eb", originX: "center", originY: "center"
  }));
  const addCircle = () => add(new fabric.Circle({
    left: QP_W / 2, top: QP_H / 2, radius: 50, fill: "#e5e7eb", originX: "center", originY: "center"
  }));
  const addLine = () => add(new fabric.Line([50, QP_H / 2, QP_W - 50, QP_H / 2], {
  ...
  const handleExport = async () => {
    ...
    toast.loading("Exporting...", { id: "dl" });
    try {
      if (format === "pdf") await exportPDF(presentation, QP_W, QP_H);
      else if (format === "pptx") await exportPPTX(presentation, QP_W, QP_H);
      else await exportImages(presentation, format as any, QP_W, QP_H);
      toast.success("Export successful!", { id: "dl" });
    } catch (e: any) {
  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground">
      ...
      <div className="flex flex-1 flex-col md:flex-row overflow-hidden">
              ...
              >
                ...
                <div className="aspect-[1/1.414] w-full bg-white">
                  {thumbs[s.id] || s.thumbnail ? (
                    ...
                    <div className="flex h-full items-end justify-center pb-1 text-[8px] text-black/60">
                      Satyabrat Mishra
                    </div>
                  )}
                </div>
        ...
        {/* Center: canvas */}
        <main className="flex flex-1 flex-col overflow-hidden bg-white/5">
          {/* Toolbar */}
      ...
      <Dialog open={downloadOpen} onOpenChange={setDownloadOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleExport} className="btn-brand">Download</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
